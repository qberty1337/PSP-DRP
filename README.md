...
2. **Desktop / PSP Companion** - A desktop app built in Rust that receives PSP game data and updates Discord Rich
...
- **Bidirectional Sync** - automatically sync latest game data usage between PSP and desktop companion
- **USB Mode** - enable game data syncing over USB to the desktop companion (see [USB Mode](#usb-mode))
...
- WiFi connection (preferably on the same network as the desktop app for online mode)
...
3. Copy all PRX files to `ms0:/SEPLUGINS/pspdrp/`:
   ...
   - `psp-plugin/usb/psp_drp_usb.prx`
4. Update `ms0:/SEPLUGINS/plugins.txt` and add:
   ...
   ```
   (Do not add `psp_drp_net.prx` or `psp_drp_usb.prx` to plugins.txt - they are loaded automatically, also do not include the xmb line if you dont want XMB browsing/idle statuses)
5. Create config file `ms0:/SEPLUGINS/pspdrp/psp_drp.ini` or use the psp_drp.ini that is bundled in the release:
   ...
   usb_mode = 0 ; set to 1 if you want to use USB Mode
   offline_mode = 0 ; set to 1 if you want to use Offline Mode (no network connection needed)
   psp_name = My PSP
   ```
6. (Optionally) Copy the `PSP DRP` folder to `ms0:/PSP/GAME/` to view your stats locally

7. Restart your PSP

8. Extract the desktop companion app, run it once to generate the config file, then edit it (or use the bundled config.toml):
...
| `usb_mode`       | Enable USB Mode (see [USB Mode](#usb-mode))                             | `0`     |
...
| Option        | Description     | Default |
| ------------- | --------------- | ------- |
| `usb.enabled` | Enable USB mode | `false` |
...
For users who want to track game usage **without WiFi**, enable USB mode:

```ini
usb_mode = 1
```

Then enable usb mode in the desktop companion's config:
...
- **Syncs usage data to desktop** - syncs play time data to the desktop app when connected via USB

This is useful for:

- PSPs without WiFi access or with the WiFi switch off
- Games that conflict with network initialization
- Users who prefer to keep their PSP offline
...
Using USB mode so far detects the most games I've tested with without freezing or crashing the PSP. The only downside is that you have to have your PSP connected to your PC via USB while playing. Or you can use offline mode and then later sync it with the desktop app (the usage log file is written to ms0:/seplugins/pspdrp/usage.json and is readable by both the desktop app and the homebrew psp app)
