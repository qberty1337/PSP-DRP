...
int network_init(void) {
    ...
    if (ret < 0 && ret != NET_MODULE_ALREADY_LOADED && ret != NET_LIBRARY_ALREADY_LOADED) { /* Already loaded */
...
int network_connect(const PluginConfig *config) {
    ...
    net_log("network_connect begin ip=%s port=%d auto=%d", config->desktop_ip, config->port, config->auto_discovery);
    ...
    {
        int enable = 1;
        int opt = sceNetInetSetsockopt(g_socket, SOL_SOCKET, SO_BROADCAST, &enable, sizeof(enable));
    ...
    if (config->desktop_ip[0] != '\0') {
        ...
        if (g_discovery_socket >= 0) {
            ...
            ret = sceNetInetBind(g_discovery_socket, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
...
static int connect_to_ap(void) {
    ...
    net_log("connect_to_ap connect ret=0x%08X profile=%d", (unsigned int)ret, g_profile_id);
    if (ret < 0) {
        return ret;
    }
...
static int send_packet(uint8_t type, const void *payload, size_t payload_size) {
    ...
    if (g_socket < 0 || g_desktop_addr.sin_addr.s_addr == 0) {
        net_log("send_packet failed: socket=%d addr=0x%08X", g_socket, (unsigned int)g_desktop_addr.sin_addr.s_addr);
        return -1;
    }
    ...
    if (config != NULL) {
        ...
        if (strcmp(config->desktop_ip, ip_str) != 0 ||
            ...
            config->auto_discovery = 0;
            {
                ...
                net_log("config_save result=%d ip=%s port=%d auto=%d", save_res, config->desktop_ip, config->port, config->auto_discovery);
            }
        }
    }
